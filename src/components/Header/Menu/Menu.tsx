...
const Menu: React.FC = () => {
  /**
   * The state of the menu logo
   */
  const [menulogo, setMenuLogo] = React.useState(true);
  /**
   * The state of the selected language
   */
  const [selectedlang, setSelectedLang] = React.useState('SP');
  /**
   * The state of the switcher
   */
  const [switcher, setSwitcher] = React.useState(false);
  /**
   * The state of the show flag
   */
  ...
  /**
   * The degree of rotation based on the selected language
   */
  let deg = 0;
  if (showFlag === 'SP') {
    deg = 0;
  ...
  return (
    <div className="menu">
      <div className="menu__lang">
        <div className="menu__lang_choose" onClick={() => setSwitcher(!switcher)}>
          {selectedlang}
        </div>
      </div>
      {/* Container for language selection */}
      <div className={switcher ? 'menu-choose' : 'none'}>
        {langs.map((item: string) => (
          <div key={item}>
            <div
              className="menu-choose__list"
              onClick={() => {
                ...
                // Reset the show flag
                setShowFlag('');
                // Dispatch an action with the selected language index
                dispatch(incrementByAmount(langs.indexOf(item)));
              }}
              onMouseEnter={() => {
                // Set the show flag on mouse enter
                setShowFlag(item);
              }}
              onMouseLeave={() => {
                // Reset the show flag on mouse leave
                setShowFlag('');
              }}>
              {item}
            </div>
      ...
      {/* Container for the flag SVGs that rotate based on selected language */}
      <div className={showFlag ? 'menu-choose-check' : 'menu-choose-check-none'}>
        <div
          style={{ transform: `rotateY(${deg}deg)`, transition: `600ms ease all` }}
          className="menu-choose-check__flag">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="menu_check_spain"
            width="66"
            height="66"
      ...
      <div>
        {/* The menu-logo div contains the 3 horizontal lines that open the menu when clicked. 
            When the menu is open, the lines rotate 90 degrees to form a cross. */}
        <div className="menu-logo" onClick={() => setMenuLogo(!menulogo)}>
          {/* The material class is the container for the 3 horizontal lines. When the menu is open, 
              this container rotates 90 degrees. */}
          <div className={menulogo ? 'menu-logo__material' : 'menu-logo__material_rotated'}>
            {/* The lines are the 3 horizontal lines that open the menu when clicked. */}
